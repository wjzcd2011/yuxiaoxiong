...
function updateGradeUI() {
  ...
  const exercisesTag = document.getElementById("exercisesTag");
  ...
  // 保存当前选中的运算类型
  const currentOp = operator.value;

  // 获取运算类型选择区域的元素
  const operationOptions = document.querySelectorAll(".operation-option");
  const multiplyOption = document.querySelector('[data-operation="multiply"]');
  const divideOption = document.querySelector('[data-operation="divide"]');
...
`;
  ...
  switch (currentGrade) {
    case 1:
      ...
      // 只允许加减
      allowedOps = ["add", "subtract"];
      multiplicationTag.classList.add("disabled");
      divisionTag.classList.add("disabled");
      // 禁用乘法和除法的运算选项
      if (multiplyOption) {
        multiplyOption.classList.add(
          "disabled",
          "opacity-50",
          "cursor-not-allowed"
        );
        multiplyOption.classList.remove(
          "selected",
          "bg-blue-100",
          "border-blue-500"
        );
        multiplyOption.style.pointerEvents = "none";
      }
      if (divideOption) {
        divideOption.classList.add(
          "disabled",
          "opacity-50",
          "cursor-not-allowed"
        );
        divideOption.classList.remove(
          "selected",
          "bg-blue-100",
          "border-blue-500"
        );
        divideOption.style.pointerEvents = "none";
      }
      break;
    ...
    case 2:
      ...
      // 允许加减乘，禁用除法
      allowedOps = ["add", "subtract", "multiply"];
      divisionTag.classList.add("disabled");
      // 禁用除法的运算选项
      if (divideOption) {
        divideOption.classList.add(
  ...
  operator.querySelectorAll("option").forEach((option) => {
    if (!allowedOps.includes(option.value)) {
      option.disabled = true;
    }
  });

  // 确定新的运算类型 - 优先保留原有类型(如果仍被允许)
  let newOp = allowedOps.includes(currentOp) ? currentOp : allowedOps[0];
  operator.value = newOp;

  // 更新运算选项的选中状态
  const selectedOption = document.querySelector(`[data-operation="${newOp}"]`);
  if (selectedOption) {
    selectedOption.classList.add("selected");
  }

  // 根据当前运算类型更新标签
  updateOperationLabels(newOp, num1Label, num2Label, processTitle);
...
// 根据运算类型更新标签显示
function updateOperationLabels(operation, num1Label, num2Label, processTitle) {
  switch (operation) {
    case "add":
...
function calculateMultiplicationLineWidth() {
  ...
  const dynamicWidth = Math.max(maxWidth + 10, 200);
...
// 手动模式下显示指定步骤的动画
// 修改显示步骤动画函数（script.js #startLine: 813 #endLine: 871 附近）
// 手动模式下显示指定步骤的动画
function showStepAnimation(stepIndex) {
  ...
  // 重置所有动画元素（保持初始隐藏）
  verticalEl
    .querySelectorAll(".carry")
    .forEach((carry) => carry.classList.remove("visible"));
  verticalEl
    .querySelectorAll(".partial-product")
    .forEach((pp) => pp.classList.remove("visible"));
  // 始终移除结果可见类，确保结果不会提前显示
  verticalEl
  ...
  if (adjustedIndex < carryDetails.length) {
    ...
    currentCarryElements.forEach((carry) => carry.classList.add("visible"));
  }

  // 手动模式：仅显示已验证通过的部分积（当前步骤之前的）
  const verifiedSteps = window.verifiedMultiplicationSteps || 0;

  for (let i = 0; i < verifiedSteps && i < partProducts.length; i++) {
...
function updateStepDisplay() {
  ...
  if (currentMode === "manual") {
    ...
    if (operator === "multiply") {
      ...
      const manualInput = document.getElementById("manualInput"); // 输入框元素
      const checkAnswerBtn = document.getElementById("checkAnswer"); // 检查答案按钮

      if (currentStepIndex === 0) {
        // 第一步：显示引导性提示，隐藏输入框和检查按钮
        document.getElementById(
          "manualInputLabel"
        ).textContent = `准备开始计算 ${num1} × ${num2}，点击下一步开始逐位计算`;
        // 隐藏输入框和按钮
        manualInput.style.display = "none";
        checkAnswerBtn.style.display = "none";
      } else if (currentStepIndex < calculationSteps.length - 1) {
        // 中间步骤：显示输入框和检查按钮，更新提示
        // 显示输入框和按钮
        manualInput.style.display = "inline-block"; // 恢复显示（根据原有样式调整）
        checkAnswerBtn.style.display = "inline-block"; // 恢复显示（根据原有样式调整）
        if (currentStep.digit2) {
          document.getElementById(
            "manualInputLabel"
          ).textContent = `请计算：${num1} × ${currentStep.digit2} = ?（注意错位）`;
        } else if (currentStep.partProduct === 0 && currentStep.digit2 === 0) {
          document.getElementById(
            "manualInputLabel"
          ).textContent = `乘数末尾有0，直接补0：${currentStep.partProduct}`;
        }
      } else {
        // 最后一步：显示输入框和检查按钮，更新提示
        manualInput.style.display = "inline-block";
        checkAnswerBtn.style.display = "inline-block";
        document.getElementById(
          "manualInputLabel"
        ).textContent = `请计算所有部分积的总和，得到最终结果：${num1} × ${num2} = ?`;
      }
      ...
      if (currentStepIndex === 0) {
        ...
        // 中间步骤：显示输入框和检查按钮，提示当前步骤
        manualInput.style.display = "inline-block"; // 恢复显示（根据实际样式调整）
        checkAnswerBtn.style.display = "inline-block"; // 恢复显示
        document.getElementById(
          "manualInputLabel"
        ).textContent = `请输入第${currentStepIndex}步的商：`;
      } else {
        // 最后一步：显示输入框和检查按钮，提示最终结果
        manualInput.style.display = "inline-block";
        checkAnswerBtn.style.display = "inline-block";
        document.getElementById(
          "manualInputLabel"
...
// 检查手动输入答案
// 修改检查手动输入答案函数（script.js #startLine: 1141 #endLine: 1237 附近）
// 检查手动输入答案
function checkManualAnswer() {
  ...
  if (operator === "multiply") {
    ...
    if (currentStepIndex === calculationSteps.length - 1) {
      ...
      // 中间步骤：校验当前位的商
      correctAnswer =
        calculationSteps[currentStepIndex - 1].product /
  ...
  if (Math.abs(userAnswer - correctAnswer) < 0.001) {
    ...
    if (operator === "multiply" && currentStepIndex > 0) {
      window.verifiedMultiplicationSteps =
        window.verifiedMultiplicationSteps || 0;
      window.verifiedMultiplicationSteps++;
...
function renderSubtractionVertical(num1, num2, data) {
  ...
  for (let i = 0; i < padded1.length; i++) {
    ...
    num1Html += `<div class="minuend-digit">${
      isBorrowed
        ? '<span style="text-decoration: line-through;">' + // 移除了 font-size: 0.8rem
          padded1[i] +
          "</span>"
        : padded1[i]
    }</div>`;
...
${borrowHtml}
<div style="margin-left: ${num1Offset}px">${num1Html}</div>
<div style="margin-left: 0px">-${num2Html}</div>

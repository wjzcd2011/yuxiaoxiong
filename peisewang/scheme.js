...
import { schemeConfigs } from "https://cdn.jsdelivr.net/gh/wjzcd2011/yuxiaoxiong/peisewang/constants.js";
import {
  getColorById,
  addColor,
  getActiveGroupId,
...
import { addToHistory } from "https://cdn.jsdelivr.net/gh/wjzcd2011/yuxiaoxiong/peisewang/history.js";

let activeSchemeTab = "complementary";

export function getActiveSchemeTab() {
  return activeSchemeTab;
}
...
export function renderSchemePanel(
  ...
  for (const [key, cfg] of Object.entries(schemeConfigs)) {
    tabsHtml += `<button class="scheme-tab ${activeSchemeTab === key ? "active" : ""
    ...
    rowsHtml += `
            <div class="scheme-row ${activeSchemeTab === key ? "active" : ""
      }" data-row="${key}">
          ...
          return `
                            <div class="scheme-swatch-card" data-hex="${hex}">
                                ...
                                <div class="scheme-swatch-info">
                                    <div class="scheme-swatch-name">${name.n || "配色色"
            }</div>
  ...
  wrap.innerHTML = `
        <div class="scheme-base-indicator">
            <div class="scheme-base-swatch" style="background:${curr.hex
    }"></div>
            <div class="scheme-base-text">基于主色：<span class="scheme-base-name">${curr.name || "未命名"
    } ${curr.hex}</span></div>
        </div>
        ${tabsHtml}
        ${rowsHtml}

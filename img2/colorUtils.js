// 颜色处理工具函数
import { formatList } from '../constants.js';
...
function toPinyinSlug(chineseName) {
    ...
    // 降级方案：简单映射
    const pinyinMap = {
        '红': 'hong', '粉': 'fen', '蓝': 'lan', '绿': 'lv', '黄': 'huang', '紫': 'zi',
        '黑': 'hei', '白': 'bai', '灰': 'hui', '橙': 'cheng', '青': 'qing', '棕': 'zong',
        '褐': 'he', '金': 'jin', '银': 'yin', '深': 'shen', '浅': 'qian', '暗': 'an',
        '亮': 'liang', '淡': 'dan', '浓': 'nong', '标准': 'bz', '复古': 'fg', '莫兰迪': 'mld',
        '马卡龙': 'mkl', '薄雾': 'bw', '云雾': 'yw', '冰川': 'bc', '水': 'shui', '玉': 'yu',
        '石': 'shi', '墨': 'mo', '鼠': 'shu', '茶': 'cha', '杏': 'xing', '桃': 'tao',
        '樱': 'ying', '玫': 'mei', '豆': 'dou', '藕': 'ou', '香': 'xiang', '奶': 'nai',
        '蜜': 'mi', '柚': 'you', '柠': 'ning', '草': 'cao', '竹': 'zhu', '松': 'song',
        '柳': 'liu', '荷': 'he', '苔': 'tai', '藤': 'teng', '萝': 'luo', '薇': 'wei',
        '槿': 'jin', '莲': 'lian', '翡翠': 'fc', '薄荷': 'bh', '草莓': 'cm', '西瓜': 'xg',
        '蜜桃': 'mt', '樱桃': 'yt', '番茄': 'fq', '爱丽丝': 'als', '巴黎': 'bl', '北京': 'bj'
    };
...
export function initFormatFunctions() {
    formatList.forEach(item => {
        ...
        else if (item.key === "rgb") item.fn = h => { const { r, g, b } = hexToRgb(h); return `rgb(${r}, ${g}, ${b})`; };
        else if (item.key === "rgba") item.fn = h => { const { r, g, b } = hexToRgb(h); return `rgba(${r}, ${g}, ${b}, 1)`; };
        else if (item.key === "hsl") item.fn = h => { const { h: hue, s, l } = hexToHsl(h); return `hsl(${Math.round(hue)}, ${Math.round(s)}%, ${Math.round(l)}%)`; };
        else if (item.key === "hsla") item.fn = h => { const { h: hue, s, l } = hexToHsl(h); return `hsla(${Math.round(hue)}, ${Math.round(s)}%, ${Math.round(l)}%, 1)`; };

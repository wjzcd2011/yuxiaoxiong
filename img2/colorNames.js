// 颜色名称数据
import { BUILTIN_COLOR_NAMES } from './constants.js';
...
export function getColorName(hex) {
    ...
    let minDiff = Infinity, closestName = "";
...
export async function loadColorNames() {
    try {
        ...
        if (resp.ok) {
            ...
            console.log(`✅ 颜色名称数据加载成功，共 ${Object.keys(colorNames).length} 条记录`);
        } else {
            console.warn('⚠️ colorNames.json 加载失败，使用内置后备数据');
        }
    } catch (err) {
        console.warn('⚠️ 无法加载 colorNames.json，使用内置后备数据。', err.message);
    }
}

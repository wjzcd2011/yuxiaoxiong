...
export function medianCut(pixels, numColors) {
    ...
    while (buckets.length < numColors) {
        let maxRange = -1, maxIdx = 0, exhausted = true;
        ...
        let rR=maxR-minR, gR=maxG-minG, bR=maxB-minB;
        let channel = 0;
        ...
        let left = bucket.slice(0,mid), right = bucket.slice(mid);
        buckets.splice(maxIdx, 1, left, right);
    }
    return buckets.map(b => {
        let sumR=0,sumG=0,sumB=0;
        b.forEach(p=>{ sumR+=p[0]; sumG+=p[1]; sumB+=p[2]; });
        return [Math.round(sumR/b.length), Math.round(sumG/b.length), Math.round(sumB/b.length)];
    }).sort((a,b)=> hexToHsl(rgbToHex(a[0],a[1],a[2])).h - hexToHsl(rgbToHex(b[0],b[1],b[2])).h);
}

export function extractColorsFromImage(imgEl, numColors) {
    const MAX = 180;
    let sw = imgEl.naturalWidth, sh = imgEl.naturalHeight;

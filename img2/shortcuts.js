// 键盘快捷键
import { closeAllModals, confirmOperate } from './modal.js';
import { getSelectedColorId, setSelectedColorId, deleteColor, getColorById, getColors, getActiveGroupId } from './colors.js';
import { openColorModal } from './modal.js';
...
export function initKeyboardShortcuts(onRenderComplete) {
    document.addEventListener('keydown', e => {
        ...
        if (e.key === 'Escape') {
            ...
            const isMultiSelectMode = multiBtn && multiBtn.innerHTML.includes('退出多选');
        ...
        if ((e.key === 'Delete' || e.key === 'Backspace') && getSelectedColorId()) {
            ...
            if (c) {
                confirmOperate(`确定删除「${c.name}」？`, () => {
                    ...
                    setSelectedColorId(remainingInGroup.length > 0 ? remainingInGroup[0].id : null);

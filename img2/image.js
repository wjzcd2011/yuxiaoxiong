...
import { getState, setExtractedColorsData, setSelectedExtractedIndices, setCurrentImageDataUrl, addColor } from './store.js';
import { extractColorsFromImage } from './algorithm.js';
import { getColorName, showToast } from './colorUtils.js';
...
function renderExtractedColors() {
    ...
    extractedColorsData.forEach((hex, idx) => {
        ...
        chip.className = `extracted-color-chip ${selectedExtractedIndices.has(idx) ? 'selected' : ''}`;
...
export function loadImageFromFile(file) {
    ...
    reader.onload = e => {
        ...
        dom.imagePreviewImg.onload = () => processUploadedImage(dom.imagePreviewImg);
    };
...
export function bindImageModalEvents() {
    ...
    dom.imagePickFile.onchange = e => {
        ...
        if(file) { dom.imagePickModal.style.display = 'flex'; loadImageFromFile(file); }
        e.target.value = '';
    };
    dom.closeImagePickModalBtn.onclick = () => dom.imagePickModal.style.display = 'none';
    dom.extractCountSlider.oninput = () => { dom.extractCountBadge.textContent = dom.extractCountSlider.value; };
    dom.extractCountSlider.onchange = () => {
        const img = dom.imagePreviewImg;
        if(img && img.src && img.style.display !== 'none' && img.complete && img.naturalWidth) processUploadedImage(img);
    };
    dom.reExtractBtn.onclick = () => {
        const img = dom.imagePreviewImg;
        if(img && img.src && img.style.display !== 'none' && img.complete && img.naturalWidth) { processUploadedImage(img); showToast('已重新提取'); }
    ...
    dom.addExtractedColorsBtn.onclick = () => {
        const { extractedColorsData, selectedExtractedIndices, activeGroupId } = getState();
        if(!extractedColorsData.length) { showToast('没有可添加的颜色'); return; }
        if(!selectedExtractedIndices.size) { showToast('请至少选择一个颜色'); return; }
    ...
    dom.imagePreviewWrap.addEventListener('dragover', e => { e.preventDefault(); e.stopPropagation(); dom.imagePreviewWrap.style.borderColor = 'rgba(126,184,255,0.5)'; });
    dom.imagePreviewWrap.addEventListener('dragleave', e => { e.preventDefault(); e.stopPropagation(); dom.imagePreviewWrap.style.borderColor = ''; });
    dom.imagePreviewWrap.addEventListener('drop', e => {
        e.preventDefault(); e.stopPropagation(); dom.imagePreviewWrap.style.borderColor = '';
    ...
    document.addEventListener('paste', e => {
        ...
        for(let item of items) {
            if(item.type.startsWith('image/')) {
                ...
                if(file) { loadImageFromFile(file); showToast('已粘贴剪贴板图片'); return; }
            }
        }
    });
}
import { getDom } from "https://cdn.jsdelivr.net/gh/wjzcd2011/yuxiaoxiong/img2/psdom.js";
import {
  getState,
  setMultiSelectMode,
  clearMultiSelected,
  ...
  applyImport,
} from "https://cdn.jsdelivr.net/gh/wjzcd2011/yuxiaoxiong/img2/psstore.js";
import {
  copyText,
  showToast,
...
import {
  openColorModal,
  confirmOperate,
} from "https://cdn.jsdelivr.net/gh/wjzcd2011/yuxiaoxiong/img2/psmodals.js";

export function registerEventHandlers() {
  const dom = getDom();

  dom.addColorBtn.onclick = () => openColorModal();
  dom.addGroupBtn.onclick = () => {
    dom.addGroupInputWrap.style.display = "flex";
  ...
  dom.newGroupName.onkeydown = (e) => {
    if (e.key === "Enter") dom.confirmAddGroup.click();
    if (e.key === "Escape") dom.addGroupInputWrap.style.display = "none";
  };
  dom.searchInput.oninput = () =>
    import(
      "https://cdn.jsdelivr.net/gh/wjzcd2011/yuxiaoxiong/img2/psrender.js"
    ).then(({ renderColors }) => renderColors());
  ...
  dom.multiSelectBtn.onclick = () => {
    ...
    clearMultiSelected();
    dom.multiSelectBtn.innerHTML = !isMultiSelectMode
  ...
  dom.exportBtn.onclick = () => {
    ...
    const blob = new Blob([JSON.stringify({ groups, colors }, null, 2)], {
  ...
  dom.importMergeBtn.onclick = () => {
    dom.importMergeBtn.classList.add("active");
    dom.importReplaceBtn.classList.remove("active");
  };
  dom.importReplaceBtn.onclick = () => {
    dom.importReplaceBtn.classList.add("active");
    dom.importMergeBtn.classList.remove("active");
  };
  dom.importConfirmBtn.onclick = () => {
    const mode = dom.importMergeBtn.classList.contains("active")
      ? "merge"
      : "replace";
    applyImport(mode);

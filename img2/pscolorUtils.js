...
export async function loadColorNames() {
  try {
    const resp = await fetch(
      "https://cdn.jsdelivr.net/gh/wjzcd2011/yuxiaoxiong/img2/colorNames.json"
    );
    if (resp.ok) colorNames = await resp.json();
    else colorNames = BUILTIN_COLOR_NAMES;
  } catch {
    colorNames = BUILTIN_COLOR_NAMES;
  }
}
...
export function interpolateColor(start, end, ratio) {
  const s = hexToRgb(start),
    e = hexToRgb(end);
  return rgbToHex(
    Math.round(s.r + (e.r - s.r) * ratio),
    Math.round(s.g + (e.g - s.g) * ratio),
...
export function generateTintShade(baseHex) {
  ...
  const map = {};
  map[50] = hslToHex(h, s, 96);
  map[100] = hslToHex(h, s, 90);
  map[200] = hslToHex(h, s, 80);
  map[300] = hslToHex(h, s, 70);
  map[400] = hslToHex(h, s, 60);
  map[500] = baseHex;
  map[600] = hslToHex(h, s, 48);
  map[700] = hslToHex(h, s, 36);
  map[800] = hslToHex(h, s, 24);
  map[900] = hslToHex(h, s, 16);
  map[950] = hslToHex(h, s, 8);
  return map;
}
...
export const formatList = [
  ...
  },
  {
    ...
    name: "CSS变量",
    fn: (h) => {
      let { n } = getColorName(h);
      return `--color-${n || "color"}: ${h};`;
    },
  },
  {
    key: "tw",

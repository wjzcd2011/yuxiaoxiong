// 渐变面板管理
import { hexToHsl, hslToHex, interpolateColor, generateTintShade, calcContrastRatio, getWcagLevel, formatColorByType, toPinyinSlug } from '../utils/colorUtils.js';
import { copyText, showToast } from '../utils/domUtils.js';
import { shadeLevels, formatList } from '../constants.js';
import { getCurrentFormat, setCurrentFormat, getColorById } from './colors.js';
...
function generateShadeCode(colorName, map, fmt) {
    ...
    if (fmt === 'css') {
        return `:root {\n` + levels.map(lv => `  --color-${finalName}-${lv}: ${map[lv]};`).join('\n') + '\n}';
    } else if (fmt === 'tailwind') {
        const entries = levels.map((lv, idx) => {
...
export function renderGradientPanel(selectedColorId, currentFormat, onFormatChange, onShadeExport) {
    ...
    wrap.innerHTML = `
        ...
        <div class="color-readout">
            ...
            <div class="readout-info">
                ...
                <div class="readout-value" id="currentColorValue" style="font-family: monospace; font-size: 13px;">--color-${getColorNameForHex(baseHex)}: ${baseHex};</div>
            </div>
            <div class="format-wrap">
                <select class="format-select" id="formatSelect">
                    ${formatList.map(f => `<option value="${f.key}" ${currentFormat === f.key ? "selected" : ""}>${f.name}</option>`).join("")}
                </select>
            </div>
        ...
        <div class="contrast-panel">
            ...
            <div class="contrast-row">
                <div class="contrast-card">
                    ...
                    <div class="contrast-value">${ratioWhite.toFixed(2)} : 1</div>
                    <div>
                        <span class="contrast-tag ${wLevel.aa ? 'tag-pass' : 'tag-fail'}">AA ${wLevel.aa ? '合规' : '不通过'}</span>
                        <span class="contrast-tag ${wLevel.aaa ? 'tag-pass' : 'tag-warn'}">AAA ${wLevel.aaa ? '合规' : '不通过'}</span>
                    </div>
                </div>
                <div class="contrast-card">
                    <div style="font-size:12px;color:var(--text-muted)">黑色背景</div>
                    <div class="contrast-value">${ratioBlack.toFixed(2)} : 1</div>
                    <div>
                        <span class="contrast-tag ${bLevel.aa ? 'tag-pass' : 'tag-fail'}">AA ${bLevel.aa ? '合规' : '不通过'}</span>
                        <span class="contrast-tag ${bLevel.aaa ? 'tag-pass' : 'tag-warn'}">AAA ${bLevel.aaa ? '合规' : '不通过'}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    if (formatSelect) {
        formatSelect.onchange = e => {
            ...
            const formatted = formatColorByType(currentHoverColor || baseHex, newFormat, currColor.name);
            const valueSpan = document.getElementById("currentColorValue");
    ...
    if (copyBtn) {
        copyBtn.onclick = () => {
            ...
            if (currentFormatValue === 'css') {
                ...
                copyText(formatColorByType(currentHoverColor, currentFormatValue, currColor.name));
            }
        };
    }

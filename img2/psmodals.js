...
export function openColorModal(editId = null) {
  ...
  dom.modalGroupSelect.innerHTML = groups
...
export function openShadeExportModal(colorObj, map) {
  ...
  shadeExportFmt = "css";
  const dom = getDom();
  dom.shadeExportCode.textContent = generateShadeCode(
...
export function bindModalButtons() {
  ...
  dom.copyShadeCodeBtn.onclick = () =>
    copyText(dom.shadeExportCode.textContent);
  dom.closeShadeExportBtn.onclick = () =>
    (dom.shadeExportModal.style.display = "none");
}
